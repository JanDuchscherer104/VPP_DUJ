...
TEST(KomponentenListTest, CheckChain) {
    ...
    kl.push_back(&b);
    kl.push_back(&f);
    KomponentenElement* m_first = kl.begin();
    KomponentenElement* second = m_first->next;
    KomponentenElement* third = second->next;

    EXPECT_NE(nullptr, m_first);
    EXPECT_EQ(nullptr, m_first->before);
    EXPECT_NE(nullptr, second);
    EXPECT_EQ(m_first, second->before);
    EXPECT_EQ(nullptr, third);

    EXPECT_TRUE(1 == kl.erase(1));  // letztes Element geloescht
    EXPECT_NE(nullptr, m_first);
    EXPECT_EQ(nullptr, m_first->before);
    EXPECT_EQ(nullptr, m_first->next);
