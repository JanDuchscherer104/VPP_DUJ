...
int main(void) {
    ...
    const char fn[] = "../werkstueck0.json";
    Serializer ser(w0);
    try {
        ser.writeToJson(fn);
    } catch (const std::exception e) {
        //} catch (const std::fstream::failure eOpen) {
        std::cout << "Warning: Runtime error occured while opening or writing to file " << fn << "." << std::endl;
